...
ORGate::ORGate(double thresholdVoltage)
    : Component(ComponentType::ORGate, 5, thresholdVoltage)
{
}

QString ORGate::valueString() const
{
    return QString::number(m_value, 'g', 3) + " " + valueUnit();
}

void ORGate::stampDC(Eigen::MatrixXd& G, Eigen::VectorXd& rhs, int /*size*/) const
{
    stampWithState(G, rhs, OutputHigh);
}

void ORGate::stampAC(Eigen::MatrixXcd& Y, Eigen::VectorXcd& /*rhs*/,
...
void ORGate::stampWithState(Eigen::MatrixXd& G, Eigen::VectorXd& /*rhs*/, State state) const
{
    int nY   = m_pins[2].nodeId - 1;
    int nVDD = m_pins[3].nodeId - 1;
    int nGND = m_pins[4].nodeId - 1;
