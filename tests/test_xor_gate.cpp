...
class TestXORGate : public QObject
{
    Q_OBJECT

private slots:
    // Test 1: XOR gate output HIGH (different inputs: A=5V, B=0V)
    // A != B → output HIGH
    void outputHigh()
    {
        auto circuit = loadCircuit("xor_gate_output_high.esim");
        ...
        QVERIFY2(vY > 4.5,
                 qPrintable(QString("V_Y: got %1 V, expected >4.5V (XOR output HIGH)")
                            .arg(vY)));
    }

    // Test 2: XOR gate output LOW (same inputs: A=5V, B=5V)
    // A == B → output LOW
    void outputLow()
    {
        auto circuit = loadCircuit("xor_gate_output_low.esim");
        QVERIFY(circuit != nullptr);
        ...
        QVERIFY2(vY < 0.5,
                 qPrintable(QString("V_Y: got %1 V, expected <0.5V (XOR output LOW)")
                            .arg(vY)));
    }

    // Test 3: XOR gate transient switching
    // Pulse on A, B=GND → XOR(A,0) = A → Y follows A
    void transientSwitching()
    {
        auto circuit = loadCircuit("xor_gate_pulse.esim");
        QVERIFY(circuit != nullptr);
        ...
        // When pulse LOW (A=0): XOR(0,0)=0 → V_Y ≈ 0V
        QVERIFY2(vMin < 0.5,
                 qPrintable(QString("V_Y_min: got %1 V, expected <0.5V").arg(vMin)));

        // When pulse HIGH (A=1): XOR(1,0)=1 → V_Y ≈ 5V
        QVERIFY2(vMax > 4.5,
    ...
    {
        auto circuit = loadCircuit("xor_gate_roundtrip.esim");
        QVERIFY(circuit != nullptr);

        Component* u1 = findByName(*circuit, "U1");
        QVERIFY(u1 != nullptr);
        QCOMPARE(u1->type(), ComponentType::XORGate);

        auto* gate = static_cast<XORGate*>(u1);
        ...
        QVERIFY(u2 != nullptr);
        QCOMPARE(u2->type(), ComponentType::XORGate);

        auto* gate2 = static_cast<XORGate*>(u2);
...
QTEST_GUILESS_MAIN(TestXORGate)
#include "test_xor_gate.moc"
