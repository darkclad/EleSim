...
// ─── Helper: get output node ID (resistor pin 0 = diode cathode side) ───
...
class TestHalfwaveRectifier : public QObject
{
    Q_OBJECT
...
void TestHalfwaveRectifier::withoutFilter()
{
    // Load circuit (AC source + Diode + R_load, no filter cap)
    QString circuitPath = QString(TEST_DATA_DIR) + "/halfwave_rectifier.esim";
    auto circuit = loadCircuit(circuitPath);
    QVERIFY2(circuit != nullptr, "Failed to load halfwave_rectifier.esim");
    ...
    // Find the output node (resistor pin 0 = cathode side of diode)
    ...
    // Expected: V_out peak ≈ 4.30V (= 5.0 - 0.7V diode drop)
    // With R_on = 1Ω and R_load = 10kΩ: V_out = (V_in - Vf) * R_load / (R_load + R_on) ≈ 4.2996V
    const double expectedPeak = 4.30;
    ...
    // Negative half: V_out should be near 0 (blocked by diode)
    // With R_off = 1MΩ, a tiny leakage current exists: V_out ≈ -5 * 10k/1M ≈ -0.05V
    QVERIFY2(vMin > -0.1,
        qPrintable(QString("V_out min should be near 0 (blocked): got %1V")
            .arg(vMin, 0, 'f', 4)));

    // Find peak current through resistor
    Component* resistor = findByName(*circuit, "R1");
    ...
    // Expected I_peak ≈ 0.430mA (= 4.30V / 10kΩ)
    const double expectedIPeak = 0.430e-3;
    ...
    // Verify half-sine shape: count frames where V_out > 0.1V vs near zero
    int positiveCount = 0, nearZeroCount = 0;
    for (double v : vOut) {
        if (v > 0.1) ++positiveCount;
        else ++nearZeroCount;
    }
    // Half-wave: roughly 50% positive, 50% near zero
    double positiveRatio = static_cast<double>(positiveCount) / vOut.size();
    QVERIFY2(positiveRatio > 0.35 && positiveRatio < 0.65,
        qPrintable(QString("Half-sine shape: %1% positive (expected ~50%)")
...
void TestHalfwaveRectifier::withFilter()
{
    // Load circuit (AC source + Diode + R_load + C_filter)
    QString circuitPath = QString(TEST_DATA_DIR) + "/halfwave_filtered.esim";
    auto circuit = loadCircuit(circuitPath);
    QVERIFY2(circuit != nullptr, "Failed to load halfwave_filtered.esim");
    ...
    // Expected V_out peak ≈ 4.30V (same as without filter)
    const double expectedPeak = 4.30;
    double relErrPeak = std::abs(vMax - expectedPeak) / expectedPeak;
    QVERIFY2(relErrPeak < tolerance,
        qPrintable(QString("V_out peak: got %1, expected %2 (err %3%)")
    ...
    // V_out min should be well above 0 (cap holds charge)
    // Use exponential discharge model: V_min = V_peak * exp(-T/(R*C))
    // T = 1/f = 0.02s, RC = 10000 * 10e-6 = 0.1s
    // Actual discharge time is slightly less than T (cap starts recharging
    // before the next peak), so this is a lower bound for V_min.
    const double RC = 10000.0 * 10e-6;
    const double T = 1.0 / 50.0;
    const double expectedMin = expectedPeak * std::exp(-T / RC); // ≈ 3.52V
    const double expectedRipple = expectedPeak - expectedMin;    // ≈ 0.78V
    ...
    // DC average
    const double expectedAvg = (vMax + expectedMin) / 2.0;
...
QTEST_GUILESS_MAIN(TestHalfwaveRectifier)
#include "test_halfwave_rectifier.moc"
