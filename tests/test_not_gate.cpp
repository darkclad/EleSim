#include <QTest>
#include <QFile>
#include <QJsonDocument>
#include <QJsonObject>
#include <cmath>

#include "core/Circuit.h"
#include "core/components/NOTGate.h"
#include "simulation/MNASolver.h"

#ifndef TEST_DATA_DIR
#define TEST_DATA_DIR "."
#endif

static std::unique_ptr<Circuit> loadCircuit(const QString& filename)
{
    QString path = QString(TEST_DATA_DIR) + "/" + filename;
    QFile file(path);
    if (!file.open(QIODevice::ReadOnly)) return nullptr;
    QJsonDocument doc = QJsonDocument::fromJson(file.readAll());
    return Circuit::fromJson(doc.object());
}

static Component* findByName(Circuit& circuit, const QString& name)
{
    for (auto& [id, comp] : circuit.components()) {
        if (comp->name() == name) return comp.get();
    }
    return nullptr;
}

class TestNOTGate : public QObject
{
    Q_OBJECT

private slots:
    // Test 1: NOT gate output HIGH (input LOW)
    // VDD=5V, A=GND(0V) → Y = !A = HIGH → V_Y ≈ 5V
    // Pull-up ON (1Ω), pull-down OFF (1MΩ)
    // V_Y = 5 * R_load/(R_on + R_load) ≈ 5 * 1000/1001 ≈ 4.995V
    void outputHigh()
    {
        auto circuit = loadCircuit("not_gate_output_high.esim");
        QVERIFY(circuit != nullptr);

        circuit->buildNetlist();

        Component* r1 = findByName(*circuit, "R1");
        QVERIFY(r1 != nullptr);

        auto result = MNASolver::solveDC(*circuit);
        QVERIFY2(result.success, qPrintable(result.errorMessage));

        int nodeY = r1->pin(0).nodeId;
        QVERIFY(nodeY > 0);
        double vY = result.nodeResults[nodeY].voltage;

        QVERIFY2(vY > 4.5,
                 qPrintable(QString("V_Y: got %1 V, expected >4.5V (NOT output HIGH)")
                            .arg(vY)));
    }

    // Test 2: NOT gate output LOW (input HIGH)
    // VDD=5V, A=5V → Y = !A = LOW → V_Y ≈ 0V
    // Pull-up OFF (1MΩ), pull-down ON (1Ω)
    // V_Y = 5 * R_on/(R_off + R_on) ≈ 5 * 1/1e6 ≈ 0.000005V
    void outputLow()
    {
        auto circuit = loadCircuit("not_gate_output_low.esim");
        QVERIFY(circuit != nullptr);

        circuit->buildNetlist();

        Component* r1 = findByName(*circuit, "R1");
        QVERIFY(r1 != nullptr);

        auto result = MNASolver::solveDC(*circuit);
        QVERIFY2(result.success, qPrintable(result.errorMessage));

        int nodeY = r1->pin(0).nodeId;
        QVERIFY(nodeY > 0);
        double vY = result.nodeResults[nodeY].voltage;

        QVERIFY2(vY < 0.5,
                 qPrintable(QString("V_Y: got %1 V, expected <0.5V (NOT output LOW)")
                            .arg(vY)));
    }

    // Test 3: NOT gate transient switching
    // Pulse on A (5V, 100Hz), VDD=5V
    // When pulse HIGH: Y ≈ 0V, when pulse LOW: Y ≈ 5V
    void transientSwitching()
    {
        auto circuit = loadCircuit("not_gate_pulse.esim");
        QVERIFY(circuit != nullptr);

        circuit->buildNetlist();

        Component* r1 = findByName(*circuit, "R1");
        QVERIFY(r1 != nullptr);
        int nodeY = r1->pin(0).nodeId;
        QVERIFY(nodeY > 0);

        double timeStep = 0.00005;  // 50µs
        double totalTime = 0.02;     // 20ms (2 cycles at 100Hz)

        auto tResult = MNASolver::solveTransientFull(*circuit, timeStep, totalTime);
        QVERIFY2(tResult.success, qPrintable(tResult.errorMessage));
        QVERIFY(tResult.frames.size() > 100);

        double vMax = -1000.0;
        double vMin = 1000.0;
        int steadyStart = static_cast<int>(tResult.frames.size()) / 2;

        for (int i = steadyStart; i < static_cast<int>(tResult.frames.size()); ++i) {
            double vY = tResult.frames[i].nodeVoltages[nodeY];
            if (vY > vMax) vMax = vY;
            if (vY < vMin) vMin = vY;
        }

        // When pulse HIGH → NOT output LOW
        QVERIFY2(vMin < 0.5,
                 qPrintable(QString("V_Y_min: got %1 V, expected <0.5V (output LOW)")
                            .arg(vMin)));

        // When pulse LOW → NOT output HIGH
        QVERIFY2(vMax > 4.5,
                 qPrintable(QString("V_Y_max: got %1 V, expected >4.5V (output HIGH)")
                            .arg(vMax)));
    }

    // Test 4: Save/load roundtrip with custom Vth
    void saveLoadRoundtrip()
    {
        auto circuit = loadCircuit("not_gate_roundtrip.esim");
        QVERIFY(circuit != nullptr);

        Component* u1 = findByName(*circuit, "U1");
        QVERIFY(u1 != nullptr);
        QCOMPARE(u1->type(), ComponentType::NOTGate);

        auto* gate = static_cast<NOTGate*>(u1);
        QVERIFY2(std::abs(gate->thresholdVoltage() - 3.0) < 0.001,
                 qPrintable(QString("Vth: got %1, expected 3.0").arg(gate->thresholdVoltage())));

        // Serialize to JSON
        QJsonObject json = circuit->toJson();

        // Reload from JSON
        auto circuit2 = Circuit::fromJson(json);
        QVERIFY(circuit2 != nullptr);

        Component* u2 = findByName(*circuit2, "U1");
        QVERIFY(u2 != nullptr);
        QCOMPARE(u2->type(), ComponentType::NOTGate);

        auto* gate2 = static_cast<NOTGate*>(u2);
        QVERIFY2(std::abs(gate2->thresholdVoltage() - 3.0) < 0.001,
                 qPrintable(QString("Roundtrip Vth: got %1, expected 3.0")
                            .arg(gate2->thresholdVoltage())));
    }
};

QTEST_GUILESS_MAIN(TestNOTGate)
#include "test_not_gate.moc"
