...
private slots:
    // Test 1: AND gate output LOW (partial input: A=5V, B=0V)
    // Only one input HIGH → output LOW
    void outputLow()
    {
        auto circuit = loadCircuit("and_gate_output_low.esim");
        ...
        QVERIFY2(vY < 0.5,
                 qPrintable(QString("V_Y: got %1 V, expected <0.5V (AND output LOW)")
                            .arg(vY)));
    }

    // Test 2: AND gate output HIGH (both inputs: A=5V, B=5V)
    // Both inputs HIGH → output HIGH
    void outputHigh()
    {
        auto circuit = loadCircuit("and_gate_output_high.esim");
        QVERIFY(circuit != nullptr);
        ...
        QVERIFY2(vY > 4.5,
                 qPrintable(QString("V_Y: got %1 V, expected >4.5V (AND output HIGH)")
                            .arg(vY)));
    }

    // Test 3: AND gate transient switching
    // Pulse on A, B=5V DC → Y toggles with A
    void transientSwitching()
    {
        auto circuit = loadCircuit("and_gate_pulse.esim");
        QVERIFY(circuit != nullptr);
        ...
        // When pulse LOW (A=0): AND(0,1)=0 → V_Y ≈ 0V
        QVERIFY2(vMin < 0.5,
                 qPrintable(QString("V_Y_min: got %1 V, expected <0.5V").arg(vMin)));

        // When pulse HIGH (A=1): AND(1,1)=1 → V_Y ≈ 5V
        QVERIFY2(vMax > 4.5,
    ...
    {
        auto circuit = loadCircuit("and_gate_roundtrip.esim");
        QVERIFY(circuit != nullptr);

        Component* u1 = findByName(*circuit, "U1");
        QVERIFY(u1 != nullptr);
        QCOMPARE(u1->type(), ComponentType::ANDGate);

        auto* gate = static_cast<ANDGate*>(u1);
        ...
        QVERIFY(u2 != nullptr);
        QCOMPARE(u2->type(), ComponentType::ANDGate);

        auto* gate2 = static_cast<ANDGate*>(u2);
...
QTEST_GUILESS_MAIN(TestANDGate)
#include "test_and_gate.moc"
